...
- **Formal contract gating (ToolGate, arXiv 2601.04688):** The closest *formal* system. Provides Hoare-style pre/postconditions per tool, formally checks world-state before/after execution. But (a) requires manual contract per tool (deployment cost the authors do not remove), (b) evaluated only on ToolBench/MCP-Universe task completion, never on InjecAgent or MCPTox, (c) code is minimal (2 commits, no README at last check), and (d) not peer-reviewed at time of writing. This is our High-threat baseline B2 — and our primary head-to-head comparison.
- **Request-derived contract gating (TraceGrant, arXiv 2608.21126, Aug 2026):** The closest *intent* system and the #1 novelty threat (added 2026-09-12). Derives a POEC Contract (Permission/Obligation/Evidence/Completion) from the trusted request *before* execution, gates every tool call with evidence/provenance + single-use Effect Certificates over a stateful Obligation Ledger, and is adversarially evaluated: ASR 0.00% on AgentDojo (949) + ASB (400) across 4 backbones, utility 70–83%, white-box adaptive 2% ASR. But it is (a) never evaluated on **InjecAgent or MCPTox** (tool poisoning at registration, 353 tools), (b) deterministic + stateful — no graded semantic score, no threshold sweep, no allow/block/**escalate** band, (c) per-environment adapters, static contract verification, and contract quality validated against *hand-built reference contracts* (setup cost nonzero, unmeasured as a metric), (d) 30–40% end-to-end latency overhead, (e) preprint with no public code. This paper bounds our novelty claim: it owns AgentDojo/ASB; our gap is a graded, escalating, stateless zero-setup gate evaluated adversarially on InjecAgent (injection) and, as a separate study, on MCPTox (registration-time tool poisoning, 353 tools) — two datasets reported independently, never merged into one combined result.
- **Server-side intent authorization (IGAC, SSRN 7195899, 2026):** Second request-derived authorization layer (added 2026-09-12). Server-side intent certificate → monotone manifest narrowing → consistency gate over OpenPort; formal static-policy non-expansion. No adversarial benchmark evidence (36 ASB-shaped external trials; residual unsafe *accepted* authority 0.09–0.27 in every real-model run, blocked only from execution by draft routing); certificate generators are rules/model-classifier per deployment. Supporting-threat; reinforces the direction, does not test our pair.
...
**Research gap (one sentence):** TraceGrant (Aug 2026) and IGAC (2026) have independently validated request-derived intent authorization — TraceGrant at 0% ASR on AgentDojo/ASB — but **no published system** derives the intent contract from the user's request and is evaluated adversarially on **both injection (InjecAgent, 1,054) and registration-time tool poisoning (MCPTox, 1,348 / 353 tools)** with a graded similarity+rule scorer, a tunable threshold with an escalation band, zero per-tool authoring, and a head-to-head vs ToolGate on the same tool set.

**Why our approach is different.** It is not another detector for one attack's text pattern; it is a *policy* derived from the user's intent and enforced at the single chokepoint every attack must pass (the tool call). That makes it vector-agnostic by construction, and it replaces ToolGate's manual per-tool contracts with zero-setup automatic derivation. Vs. TraceGrant specifically: we are stateless, zero-setup, and graded (S, τ, δ, escalate) — and we run on the two benchmarks (InjecAgent, MCPTox) that TraceGrant never evaluates on, including MCPTox's 353-tool poisoning surface that no published gate covers.
...
- **Co-primary (C2):** A fair, adversarial evaluation of contract-style gating — our gate vs. a ToolGate reimplementation — as **two separate benchmark studies**: one on InjecAgent (injection), one on MCPTox (tool poisoning), each with its own ASR/FPR/latency/setup-cost tables and conclusions (never combined into a single cross-vector number), reusable by others.

**Why publication-worthy.** It closes the cross-paper gap flagged in our own synthesis: benchmarks measure vulnerability (InjecAgent, MCPTox), defenses don't cross-evaluate (ToolGate is task-completion-only; TraceGrant owns AgentDojo/ASB but never touches injection or poisoning; IGAC is unevaluated adversarially). A workshop/Findings-tier paper that runs those evaluations on InjecAgent and on MCPTox — two separate studies, a zero-setup, graded, escalating gate, and a head-to-head vs ToolGate on the same tool set — is a citable artifact, even if the gate is simple. The gate itself is a practical contribution with very-high real-world impact (pip wrapper, low-medium compute).
...
**Existing limitations.** No mechanism (a) derives policy automatically from the request (no manual authoring), (b) enforces at the action gate per vector, and (c) is tested on InjecAgent and on MCPTox as separate, independently reported studies.
...
1. Run B1 (unprotected ReAct) on InjecAgent and on MCPTox (separately) → baseline ASR, benign utility.
...
| **Research** | Reviewers see this as "just a threshold on embeddings" — incremental | Low-Medium | Moderate | Position as *evaluation* contribution: first adversarial test of request-derived contract gating as two separate studies on InjecAgent and on MCPTox (incl. 353-tool poisoning; neither TraceGrant/IGAC/ToolGate covers them) + zero-setup vs. manual cost — the mechanism's simplicity is a feature (low-medium compute) |
...
**Novelty (revised 2026-09-12 after TraceGrant + IGAC):** The idea "derive authority from the user's request and gate tool calls to it" is no longer unoccupied — TraceGrant already reports 0% ASR on AgentDojo/ASB and IGAC is a server-side variant. Our novelty is now the specific, defensible point: **no existing system** (a) is evaluated adversarially on InjecAgent injection *and* MCPTox registration-time tool poisoning (353 tools) — the benchmarks neither ToolGate nor TraceGrant tests, the poisoning surface **no published gate** tests; (b) is a graded scorer (embedding + hard veto) with a tunable τ, an ASR–FPR Pareto sweep, and a human **escalate** band — TraceGrant is deterministic deny/admit with no graded risk or user escalation; (c) is a stateless, zero-setup, model-agnostic middleware with a head-to-head ToolGate comparison on the same tool set and a per-tool setup-cost metric. ToolGate is manual + non-adversarial; TraceGrant is stateful + AgentDojo/ASB-only + hand-built reference contracts; IGAC is server-side + unevaluated adversarially; TrustAgent is hand-constitution + emulated; StruQ/CaMeL are model-level.
...
- ASR_ours < ASR_B1 on InjecAgent and on MCPTox with 95% CI not overlapping (McNemar p < 0.05), judged per benchmark (separate comparisons).
...
**Open-source framework:** The gate as a standalone middleware (LangChain/MCP wrapper) is independently valuable — other teams can drop it in front of any agent. The adversarial evaluation harness (runs InjecAgent and MCPTox, reports per-benchmark ASR/FPR/latency/setup-cost) is itself a reusable benchmark contribution, similar to how AgentDojo's harness is reused.
...
- *"Your scorer is just cosine similarity plus a few if-statements — how is this not trivial?"* — Mitigated by the evaluation framing: trivial is a feature if it works at zero setup cost. The paper's claim is not "novel math" but "the first adversarial test of request-derived contract gating, run as separate studies on InjecAgent and on MCPTox (incl. tool poisoning) without manual cost, with a graded threshold + escalation band that deterministic deny/admit gates (TraceGrant, IGAC) lack." Preempt by leading with setup-cost and per-benchmark results, not scorer novelty. Ablations (A1–A3) prove each piece matters — if they don't, the reviewer is right and we must report it.
...
**Missing datasets:** None critical — InjecAgent and MCPTox are exactly the right two datasets (injection; poisoning), matching the gap we claim, but they are different benchmark families and are evaluated and reported **separately** (separate result tables, separate ASR/FPR, separate conclusions). AgentDojo is optional stretch; do not add more benchmarks at the cost of depth.

**Possible reviewer criticisms, ranked by severity:**

1. **"How is this different from TraceGrant — it already does request-derived contract gating with 0% ASR?"** — This is the #1 question after 2026-09-12 and must be answered in the first paragraph of related work: TraceGrant is deterministic + stateful (obligation ledger, provenance, single-use certificates) with per-environment adapters and hand-built reference contracts, has no graded score, no threshold sweep, no escalation band, and — decisively — is **never evaluated on InjecAgent or MCPTox**; registration-time tool poisoning (MCPTox, 353 tools) is untested by any published gate, including TraceGrant and ToolGate. We report on exactly that pair, with a graded allow/block/escalate gate and a head-to-head vs ToolGate on the same tool set.
2. **"How is this meaningfully different from ToolGate's contracts — you just replaced formal logic with embeddings?"** — Same gate placement, opposite policy source (manual Hoare vs. auto intent), opposite evaluation (task completion vs. adversarial injection+poisoning), and we *measure* the manual cost they don't.
3. **"Your MCPTox results are on a static snapshot, not live servers — does this count?"** — Acknowledge live vs. static explicitly; argue that poisoning-at-registration logic is identical in both, and document server versions; treat as honest limitation.
4. **"FPR in single digits is still too high for deployment."** — Counter with escalate band: hard-block FPR is lower, escalate FPR is user-confirmed. Report both separately so reviewer can see the tradeoff.
...
4. Before defense: prepare the one-slide "Gate Comparison" table — **Ours vs ToolGate vs TraceGrant** (setup cost, gate placement, policy source, graded/escalate, ASR on InjecAgent, ASR on MCPTox, ASR on AgentDojo/ASB [cede to TraceGrant or cite], latency) — this is the slide the committee will remember.

---

**Blueprint complete.** All 18 sections are internally consistent with `literature_review.md` (10 papers + 2026-09-12 addendum: TraceGrant + IGAC = 12 verified), `literature_review/index.md` (Master Matrix for 5 anchors + ToolGate/TraceGrant/IGAC as closest/supporting), and Idea 1 §1–12. The only net-new additions beyond your Idea text are: the Week 1–2 pilot as explicit Go/No-Go, the escalate band as FPR safety valve, the ToolGate coverage % reporting, the threshold Pareto + error taxonomy as required figures, and the 2026-09-12 novelty narrowing forced by TraceGrant/IGAC — each flagged with rationale.
